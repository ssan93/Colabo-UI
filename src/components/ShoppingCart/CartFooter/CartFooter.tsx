...
export const CartFooter = ({ items, discounts }: CartFooterProps) => {
  ...
  const totalPrice = calculateTotalPrice(items, discounts);
  ...
  return (
    <div className="sticky max-w-screen-sm w-full bg-white bottom-0 border-t-2 px-4 py-6">
      <div className="flex items-center justify-between">
        ...
        <p className="text-4xl font-bold">{`${totalPrice} ${currency}`}</p>
      </div>
      <button className="w-full bg-blue-500 text-white text-lg rounded-lg p-2 mt-6">
        다음
      </button>
    </div>
  );
